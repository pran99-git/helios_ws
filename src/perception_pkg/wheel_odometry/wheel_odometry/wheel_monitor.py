...
#
# Reads both RoboClaws directly (NOT through the ROS node) and prints a live
# per-corner table of raw encoder counts + per-cycle deltas, plus the body twist
# the odometry kinematics would produce. Use it with the wheels OFF the ground
# to confirm wheel signs and directions.
#
# Wiring (final): ACM0 = LEFT controller (M2 front, M1 rear), ACM1 = RIGHT
# controller (M1 front, M2 rear).
#
# IMPORTANT: a serial port cannot be opened twice. Stop the wheel_odometry node
# before running this.
#
# Usage:
#   ros2 run wheel_odometry wheel_monitor
#   ros2 run wheel_odometry wheel_monitor --baud 38400
#
...
def parse_args(argv):
    p = argparse.ArgumentParser(description='Live RoboClaw encoder monitor for '
                                            'mecanum odometry calibration.')
...
def main(argv=None):
    ...
    try:
        ...
        print('Is the wheel_odometry node still running? Stop it first '
    ...
    print(f'ACM0/left={args.left_port}@{hex(args.left_address)}  '
    ...
    try:
        while True:
            ...
            if prev is None or any(v is None for v in corners.values()):
                ...
                for name in CORNERS:
                    lines.append(f'  {name:12s} count={corners[name]:>12d}   '
                                 f'delta={fmt(deltas[name])}')
                # Body twist from the mecanum kinematics (uninverted).
                d = {k: deltas[k] * meters_per_count for k in deltas}
                vx = 0.25 * (d['front_left'] + d['front_right']
                             + d['rear_left'] + d['rear_right']) / dt
                vy = 0.25 * (-d['front_left'] + d['front_right']
                             + d['rear_left'] - d['rear_right']) / dt
                wz = ((-d['front_left'] + d['front_right']
                       - d['rear_left'] + d['rear_right'])
                      / (4.0 * l_sum)) / dt
                twist_line = (f'  twist (no inversion):  '
