...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Per-motor QPPS calibration for the A4WD3 RoboClaws."
    )
    ...
    p.add_argument(
        "--pause-time", type=float, default=2.0, help="Seconds to rest between wheels."
    )
    p.add_argument(
        "--yes",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    """Measures QPPS on each of the four motor channels.
    ...
    if not args.yes:
        ...
        print(
            f"!! This will spin each motor individually up to {args.duty * 100:.0f}% duty."
        )
    ...
    def set_duty(name: str, m1: float, m2: float) -> None:
        """Applies an open-loop duty to one controller, warning if unacknowledged."""
        ok = controllers[name].duty_m1m2(m1, m2)
        if not ok:
            print(f"  WARNING: {name} duty command not acknowledged", file=sys.stderr)

    def all_stop() -> None:
        """Cuts drive on all four channels."""
    ...
    try:
        ...
        for wheel, (ctrl_name, channel) in WHEEL_SPEC.items():
            ...
            if samples:
                ...
                results[wheel] = qpps
                print(
                    f"  {wheel}: QPPS = {qpps:.0f} counts/sec "
                    f"({len(samples)} samples, min={min(samples):.0f}, max={max(samples):.0f})"
                )
            else:
                print(
                    f"  {wheel}: NO VALID SPEED READS -- check wiring/serial",
                    file=sys.stderr,
                )
    ...
    if results:
        print("\n=== Summary (paste into roboclaw.yaml) ===")
