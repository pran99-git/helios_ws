...
class TeleopJoyNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f'deadzone={self.deadzone}. Axis/button indices are UNVERIFIED '
            f'defaults -- confirm with `ros2 topic echo /joy` before trusting '
            f'them.')
    ...
    def _on_joy(self, msg):
        ...
        twist = Twist()
        held = (0 <= self.deadman_button < len(msg.buttons)
                and msg.buttons[self.deadman_button] == 1)

        if held:
            twist.linear.x = self._axis(msg, self.axis_vx, self.invert_vx)
            twist.linear.y = self._axis(msg, self.axis_vy, self.invert_vy)
            twist.angular.z = self._axis(msg, self.axis_omega, self.invert_omega)
    ...
    def _publish(self):
        ...
        stale = (self._last_joy_time is None
