...
```
teleop/
  roboclaw_driver_node.py   The ROS node. Owns the serial ports.
  ...
  wheel_monitor.py          Calibration helper: live per-corner encoder table.
  calibrate_qpps.py         Calibration helper: measures each motor's top speed.
launch/
...
config/
  teleop.yaml               Parameters for both nodes.
test/
...
The directory is called `teleop/` for historical reasons — it now holds the
whole subsystem, not just teleop.

### `teleop/roboclaw_driver_node.py`
...
### `teleop/wheel_monitor.py` and `teleop/calibrate_qpps.py`
...
Two blocks, one per node.

`teleop_joy` — axis and button indices, and the velocity at full stick
deflection:
...
`roboclaw_driver` — ports, addresses, geometry, limits:
...
running — that is two drivers on one serial port.
...
> port with `udevadm info -n /dev/ttyACM0 | grep DEVPATH` — the `1-4.x` segment
> — and update `KERNELS==` in the rule.
...
SN30 Pro reports differently in X-input, D-input and Switch modes — pick one and
